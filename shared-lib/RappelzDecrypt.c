#include <stdlib.h>
#include <stdio.h>

#include "RappelzDecrypt.h"

#define BUFFER_SIZE 16384

//xor file with decryptKey[fileToDecryptPosition]

char decryptKey[0x100] =
		{};

unsigned int decryptRappelzData(char *data, char *decrypted, int size, unsigned int decryptPos) {
	unsigned int i;

    for(i=0; i<size; i++, decryptPos++) {
		decryptPos %= 0x100;
		decrypted[i] = decryptKey[decryptPos] ^ data[i];
    }

	return decryptPos;
}

size_t cryptfread(void *ptr, size_t size, size_t count, FILE *stream) {
	int nbRead;
	unsigned int pos;

	pos = ftell(stream) % 0x100;
	
	nbRead = fread(ptr, size, count, stream);
	decryptRappelzData((char*)ptr, (char*)ptr, nbRead*size, pos);

	return nbRead;
}


size_t cryptfwrite(void *ptr, size_t size, size_t count, FILE *stream) {
	int nbRead;
	unsigned int pos;

	pos = ftell(stream) % 0x100;
	
	decryptRappelzData((char*)ptr, (char*)ptr, count*size, pos);
	nbRead = fwrite(ptr, size, count, stream);

	return nbRead;
}
