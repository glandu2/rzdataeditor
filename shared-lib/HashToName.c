#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
#include <errno.h>

#include "HashToName.h"

static char decryptLastCharTable[0x80] = 
	{};

static char decryptTablePhase2[0x80] =
	{};

void decryptPhase1(char *hash) {
	char val1, val2;
	int medianPt13 = (int)(0.33*strlen(hash));
	int medianPt23 = (int)(0.66*strlen(hash));

	val1 = hash[medianPt23];
	val2 = hash[medianPt13];

	hash[medianPt23] = hash[0];
	hash[medianPt13] = hash[1];

	hash[0] = val1;
	hash[1] = val2;
}

int decryptLastChar(char c) {
	return decryptLastCharTable[(unsigned char)c];
}

void decryptPhase2(char *hash, int decodeSeed) {
	int i, j;
	char computeVar;
	int computeLoop = decodeSeed;
	int hashSize = strlen(hash);

	for(i=0; i < hashSize; i++) {
		//décodage du caractere
		computeVar = hash[i];
		for(j=0; j < computeLoop; j++) {
			computeVar = decryptTablePhase2[(unsigned char)computeVar];
			if(!computeVar) computeVar = 0xFF;
		}
		hash[i] = computeVar;
		computeLoop = (1+computeLoop + 17*computeVar)&0x1F;
		if(!computeLoop) computeLoop = 0x20;
	}
}

/**
 * Converti un hash en son nom compréhensible par l'être humain.
 * \param hash Le hash a convertir
 * \param name buffer qui contiendra le nom associé au hash après la convertion (hash et name peuvent pointer vers le même espace mémoire)
 * \return Code de l'erreur:
 * \li \c 0 Pas d'erreur.
 * \li \c EINVAL hash ou name est NULL.
 * \sa convertNameToHash
 */
int convertHashToName(char *hash, char *name) {
	char reducedHash[260];
	
	if(!hash || !name) return EINVAL;

	strcpy(reducedHash, hash+1);
	reducedHash[strlen(reducedHash)-1] = 0; //supprime le dernier char

	decryptPhase1(reducedHash);
	decryptPhase2(reducedHash, decryptLastChar(hash[strlen(hash)-1]));
	strcpy(name, reducedHash);

	return 0;
}

/**
 * Récupère le numéro du data.00x ou sont les données du fichier associé au hash.
 * \param hash Le hash du fichier
 * \return Le numéro du fichier de donnée (1 pour data.001, 2 pour data.002 etc ...).
 */
int getAssociatedDataFileNumber(char *hash) {
	char buffer[256];
	char *p = buffer;
	int checksum=0;
	
	if(!hash) return 1;

	do {
		*(p++) = (char)tolower(*hash);
	}while(*(hash++));

	p = buffer;
	while(*p) {
		checksum = checksum*31 + *p;
		p++;
	}

	if(checksum < 0) checksum = -checksum;
	return (checksum & 0x07)+1;
}

